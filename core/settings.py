...
# Additional CORS settings to handle preflight requests properly
...
SPECTACULAR_SETTINGS = {
    ...
    "DESCRIPTION": "API For Payroll automatic system",
    "VERSION": "0.1.0",
...
# Use DATABASE_URL from Railway (or fallback to individual variables for local dev)
DATABASES = {
    "default": dj_database_url.config(
        default=f"postgresql://{os.getenv('DB_USER', 'postgres')}:{os.getenv('DB_PASSWORD', '')}@{os.getenv('DB_HOST', 'localhost')}:{os.getenv('DB_PORT', '5432')}/{os.getenv('DB_NAME', 'railway')}",
        conn_max_age=600,
...
if ENVIRONMENT == "production":
    # Note: Railway/Vercel handle SSL at the proxy level, so we don't need SECURE_SSL_REDIRECT
    # Setting it to True can cause issues with CORS preflight requests
